...
async function fetchBikeData(rental_data) {
    ...
    const time_rented = await getBikeTime('godzina wypożyczenia hulajnogi', rental_data);
    const time_returned = await getBikeTime('godzina zwrotu hulajnogi', rental_data);
...
async function fetchRentalsScooter(userId) {
    let { data, error, status } = await supabase
        .from('rental_object')
        .select(`*`)
        .eq('user_uuid', userId)
        .eq('type_name', 'wypożyczenie hulajnogi')
    if (error) {
...
export default fetchRentalsScooter;